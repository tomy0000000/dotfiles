...
- If a target can't run on CI (login required, destructive defaults, paid/licensed app, etc.), include it commented-out with a `# Reason` note rather than omitting it silently. The commented entry is the breadcrumb that explains *why* it's missing.
